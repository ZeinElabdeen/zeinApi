    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form id="change-pwd" action="{{url('dashboard/classification/update')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="avatar-upload">
                                    ...
                                    <div class="avatar-preview">
                                        <div id="imagePreview" style="background-image: url({{config('classification_storage').$data->image}});">
                                        </div>
                                    </div>
                                </div>
                                @if ($errors->has('image'))
                            ...
                            <div class="col-md-6 form-group mb-3">
                                <label for="brand"> القسم</label>
                                <select class="form-control attribute" name="brand" id="brand" >
                                    <option disabled>اختر القسم </option>
                                    @foreach($brands as $brand)
                                        <option value="{{$brand->id}}" {{($data->category_id == $brand->id)??'selected'}}>{{$brand->title}}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('brand'))
                                    <span class="text-danger" role="alert">
