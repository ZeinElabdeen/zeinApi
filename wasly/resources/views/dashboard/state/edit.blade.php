        ...
        <div class="col-md-12">
            <div class="card mb-4">
                <div class="card-header">
                    <div class="card-state mb-3">  <strong class="text-primary">تعديل محافظة {{$data->state}}</strong></div>
                </div>
                @include('dashboard.layouts.message')

                <div class="card-body">
                    <form id="change-pwd" action="{{url('dashboard/state/update')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" value="{{$data->id}}">
                        <div class="row">
                            <div class="col-md-6 form-group mb-3">
                                <label for="state">الإسم</label>
                                <input type="text" class="form-control" name="state" id="state" placeholder="الإسم " value="{{$data->state}}" autocomplete="off">
                                @if ($errors->has('state'))
