                ...
                <div class="card text-left">
                    <div class="card-header">
                        <div class="card-title mb-3">
                            <strong class="text-primary"> قائمة الأسئلة</strong>
                            <span class="align-baseline" style="display:inline;">
                                <a class="btn btn-primary " href="{{url('dashboard/question/create')}}" style="float: left">
                        ...
                        @if(count($data) > 0)
                            <div class="table-responsive">
                                <table id="alternative_pagination_table" class="display table table-striped table-bordered" style="width:100%">
                                    ...
                                    <tbody>
                                      <?php $type['1'] ="النوع الاول";
                                      $type['2'] ="النوع الثاني"; ?>
                                    @if(count($data)>0 && !empty($data))
                                        @foreach($data as $item)
                                        <?php $catname = DB::table('categories')->where('id',$item->category_id)
                                                                  ->first()->title; ?>
                                            <tr>
                                                <td>{{$item->id}}</td>
                                                <td>{{$item->title}}</td>
                                                ...
                                                <td style="text-align: center;">
                                                    <a href="{{url('dashboard/question/edit/'.$item->id)}}" class="text-info mr-2">
                                                        <i class="nav-icon i-Eye font-weight-bold " ></i>
                                                    </a>
                                                    <a href="{{url('dashboard/question/delete/'.$item->id)}}" class="text-danger mr-2">
