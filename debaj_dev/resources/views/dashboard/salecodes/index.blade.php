                                    ...
                                    @if(count($data)>0 && !empty($data))
                                        @foreach($data as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    @if( $item->statu == '1')
                                                    <a  href="{{url('dashboard/salecodes/suspend/'.$item->id)}}"  class="badge badge-success status-btn" style="width:80px;height: 25px;padding:auto;margin:auto;
                                                      padding-top: 7px; font-size: small">فعال</a>
                                                    @else
                                                    <a  href="{{url('dashboard/salecodes/suspend/'.$item->id)}}"  class="badge badge-danger status-btn" style="width:80px;height: 25px;padding:auto;margin:auto;
                                                      padding-top: 7px; font-size: small">غير فعال</a>

                                                    @endif

                                                </td>
                                                <td style="text-align: center;">
                                                    <a href="{{url('dashboard/salecodes/edit/'.$item->id)}}" class="text-info mr-2">
                                                        <i class="nav-icon i-Edit font-weight-bold " ></i>
                                                    </a>
                                                    <a href="{{url('dashboard/salecodes/delete/'.$item->id)}}" class="text-danger mr-2">
