                ...
                <div class="card text-left">
                    <div class="card-header">
                        <div class="card-title mb-3">
                            <strong class="text-primary"> قائمة الألوان</strong>
                            <span class="align-baseline" style="display:inline;">
                                <a class="btn btn-primary " href="{{url('dashboard/colors/create')}}" style="float: left">
                                <i class="i-Add align-middle" style="font-size: 17px; font-weight: 600;"></i> إضافة لون  </a>
                            </span>
                        </div>
                    </div>
                                    ...
                                    @if(count($data)>0 && !empty($data))
                                        @foreach($data as $item)
                                            <tr>
                                                ...
                                                <td>{{$item->name_ar}}</td>
                                                <td>{{$item->name_en}}</td>
                                                <td style="background-color:{{$item->code}}"> </td>
                                                <td style="text-align: center;">
                                                    <a href="{{url('dashboard/colors/edit/'.$item->id)}}" class="text-info mr-2">
                                                        <i class="nav-icon i-Edit font-weight-bold " ></i>
                                                    </a>
                                                    <a href="{{url('dashboard/colors/delete/'.$item->id)}}" class="text-danger mr-2">
