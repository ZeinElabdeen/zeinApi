...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form id="upload" action="{{url('vendor/dashboard/product/update/'.$product->id)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('put')
    ...
    <!--  <div class="row">
        ...
        <div class="col-md-6 form-group mb-3">
            ...
            <input type="number" class="form-control" step=".1" min="0.5" max="6.5" name="size_num" id="size_num" placeholder="المقاس بالمتر"  value="{{$product->size_num}}">
    ...
    if(confirm("هل انت متاكد من مسح الصورة !"))
		{
			var data = {'img_id': img_id ,'_token': '{{ csrf_token() }}'};
			 ...
			 $.ajax({
					url: '{{url("vendor/dashboard/product/image_delete")}}',
					type: 'POST',
					data:data,
					async: false,
    ...
    if(confirm("هل انت متاكد من تعين كرئيسية!"))
		{
			var data = {'img_name': img_name ,'pro_id': pro_id ,'_token': '{{ csrf_token() }}'};
			 ...
			 $.ajax({
					url: '{{url("vendor/dashboard/product/homeimg")}}',
					type: 'POST',
					data:data,
					async: false,
					success: function (data) {
