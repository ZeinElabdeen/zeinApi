        ...
        <section class="content-header">
            ...
            <div class="row mb-2">
                ...
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item active">All Online Courses</li>
                </ol>
                </div>
            </div>
            </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">DataTable with minimal features & hover styles</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example2" class="table table-bordered table-hover ">
                  ...
                  <tbody>
                   ...
                   @foreach ($allCourses as $course)
                        <tr>
                            <td>{{$course->online_course_id}}</td>
                            <td>{{$course->online_course_name}}</td>
                            <td>{{$course->institute_name}}</td>
                            <td class="text-center">
                                <a href="{{url('admin/courseOnline/'.$course->online_course_id)}}" class="btn btn-outline-warning rounded"><i class="fas fa-edit"></i></a>
                            </td>
                            <td class="text-center">
                                <form action="{{url('admin/courseOnline/'.$course->online_course_id)}}" method="post" style="display: inline">
                                    @csrf
                                    @method('DELETE')
