...
@extends('user.ar.layouts.lay') @section('title',' ملاحظات ') @section('links')
    ...
    <div class="container">
      <div class="row">
        <div class="col-lg-12 text-center">
          <div class="banner">
              {{-- <h2>About-US</h2> --}}
              <span class="text-light">الرئيسية  / <span> ملاحظات </span>
          </div>
        </div>
      </div>
    </div>
...
<div class="accordion" id="accordionExample">
    ...
    <div class="row my-3 align-items-center">
        <div class="col-9 text-right">
            <h3 class=" h1" style="color: #565656"> ملاحظاتي </h3>
        </div>
        <div class="col-3">
            <div class="text-center">
                <a href="{{url('add-note')}}" class="" style="text-decoration: none">
                    <h4 class=" d-inline" style="color: #565656">  أضف ملاحظة  <i class="fas fa-plus-circle text-warning" style="font-size:25px"></i> </h4>
                </a>
            </div>
    ...
    @if($allNotes->isEmpty())
        <h3 class="text-danger text-center my-5"> لايوجد ملاحظات حاليا </h3>
    @else
        @if(Session::has('Error'))
        <div class='alert alert-danger text-center' >{{Session::get('Error')}}</div>
        {{Session::forget('Error')}}
            ...
            <div class="card-header " id="headingOne{{$loop->index}}">
              <h2 class="mb-0">
                <form action="{{url('deleteNote/'.Session::get('user_id').'/'.$note->note_id)}}" method="post">
                  @csrf
                  <button type="submit" class="close " style="float:left !important" aria-label="Close">
            ...
            <div id="collapseOne{{$loop->index}}" class="collapse" aria-labelledby="headingOne{{$loop->index}}" data-parent="#accordionExample">
              <div class="card-body">
                  <form action="{{url('editNote/'.Session::get('user_id').'/'.$note->note_id)}}" method="post" enctype="multipart/form-data">
                        ...
                        @endif
                        <button type="submit" class="btn btn-success ml-auto d-block" > تعديل </button>
                  </form>
                  <script>
